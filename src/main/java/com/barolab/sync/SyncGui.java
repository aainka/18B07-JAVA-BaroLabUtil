...
public class SyncGui implements JYController {
	...
	public void build() {
		...
		frame.size(1500, 900);
		pan = frame.add(new JYPanel("PathView"), BorderLayout.WEST);
		{
			pan.add(new JYTable("table").size(500, 100), BorderLayout.CENTER);
			Widget box = pan.add(new JYPanel("ControlButton").setSize(100, 100), BorderLayout.SOUTH);
			{
				box.add(new JYButton("scan"), null);
				box.add(new JYButton("redo"), null);
...
//		return new DefaultTableModel(contents, title);
	...
	public void event(JYParam param) {
		...
		if (param.match("action", "scan")) {
			JYTable wtable = (JYTable) Widget.findWidget(pan, "table");
